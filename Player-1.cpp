...
int Player::getCard(int cardValue) {                            //Method removes and returns all cards of input face value
  ...
  for(int i=0; i<52; i++) {                                     //Loop through every cell of cardsInHand
    if(cardsInHand[i] > 0) {                                    //If cell holds actual card value
     int  val = cardsInHand[i]%13;                                  //Face value of card asked for
      if(val == 0)                                              //If player asked for king
        val = 13;                                               //Set val to be 13 for king
      if(val == cardValue) {                                    //If face value of card being examined matches card asked for
